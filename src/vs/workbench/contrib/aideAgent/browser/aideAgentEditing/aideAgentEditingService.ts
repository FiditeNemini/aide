...
import { ChatEditingModifiedFileEntry } from './aideAgentEditingModifiedFileEntry.js';
...
const STORAGE_KEY_EDITING_SESSION = 'aideAgent.editingSession';
...
export class ChatEditingService extends Disposable implements IAideAgentEditingService {
	...
	constructor(
		...
		this._register(bindContextKey(CONTEXT_CHAT_CAN_UNDO, contextKeyService, (r) => {
			return this._currentSessionObs.read(r)?.canUndo.read(r) || false;
		}));
		this._register(bindContextKey(CONTEXT_CHAT_CAN_REDO, contextKeyService, (r) => {
	...
	private async _createEditingSession(chatSessionId: string): Promise<IChatEditingSession> {
		...
		const session = this._instantiationService.createInstance(ChatEditingSession, chatSessionId, true, this._lookupEntry.bind(this));
		await session.init();
	...
	async createAdhocEditingSession(chatSessionId: string): Promise<IChatEditingSession & IDisposable> {
		const session = this._instantiationService.createInstance(ChatEditingSession, chatSessionId, false, this._lookupEntry.bind(this));
		await session.init();
