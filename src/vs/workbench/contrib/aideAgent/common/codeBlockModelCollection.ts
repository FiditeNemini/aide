...
export class CodeBlockModelCollection extends Disposable {
	...
	get(sessionId: string, chat: IChatRequestViewModel | IChatResponseViewModel | IAideAgentPlanStepViewModel, codeBlockIndex: number): CodeBlockEntry | undefined {
	...
	getOrCreate(sessionId: string, chat: IChatRequestViewModel | IChatResponseViewModel | IAideAgentPlanStepViewModel, codeBlockIndex: number): CodeBlockEntry {
	...
	updateSync(sessionId: string, chat: IChatRequestViewModel | IChatResponseViewModel | IAideAgentPlanStepViewModel, codeBlockIndex: number, content: CodeBlockContent): CodeBlockEntry {
	...
	markCodeBlockCompleted(sessionId: string, chat: IChatRequestViewModel | IChatResponseViewModel | IAideAgentPlanStepViewModel, codeBlockIndex: number): void {
		const entry = this._models.get(this.getKey(sessionId, chat, codeBlockIndex));
		if (!entry) {
			return;
		}
	...
	async update(sessionId: string, chat: IChatRequestViewModel | IChatResponseViewModel | IAideAgentPlanStepViewModel, codeBlockIndex: number, content: CodeBlockContent): Promise<CodeBlockEntry> {
		const entry = this.getOrCreate(sessionId, chat, codeBlockIndex);
	...
	private getKey(sessionId: string, chat: IChatRequestViewModel | IChatResponseViewModel | IAideAgentPlanStepViewModel, index: number): string {
		return `${sessionId}/${chat.id}/${index}`;
	}

	private getCodeBlockUri(sessionId: string, chat: IChatRequestViewModel | IChatResponseViewModel | IAideAgentPlanStepViewModel, index: number): URI {
